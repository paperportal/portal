...
bool wasm_api_register_gesture(void)
{
    ...
    bool ok = wasm_runtime_register_natives("portal_gesture", g_gesture_native_symbols, count);
    if (!ok) {
        ESP_LOGE(kTag, "Failed to register portal_gesture natives (count=%" PRIu32 ")", count);
